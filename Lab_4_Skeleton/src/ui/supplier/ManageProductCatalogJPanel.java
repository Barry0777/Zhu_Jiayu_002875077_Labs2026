...
public class ManageProductCatalogJPanel extends javax.swing.JPanel {
    ...
    // 取得产品对象（假设第一列存对象）
    Product prod = (Product) tblProducts.getValueAt(selectedRowIndex, 0);

    // 创建详情面板
    ViewProductDetailJPanel detailPanel =
            new ViewProductDetailJPanel(workArea, prod);

    // 添加到 CardLayout
    workArea.add("ProductDetailCard", detailPanel);

    // 切换到该卡片
    CardLayout card = (CardLayout) workArea.getLayout();
    card.show(workArea, "ProductDetailCard");
    ...
    // 从表格中取得对应的产品对象
    Product productToRemove = (Product) tblProducts.getValueAt(selectedIndex, 0);

    // 删除该产品（从所属供应商的产品目录中）
    supplier.getProductCatalog().removeProduct(productToRemove);
